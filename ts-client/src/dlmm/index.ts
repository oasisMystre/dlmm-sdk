...
export class DLMM {
  ...
  static async getPosition(
    ...
    const positionAccountInfo =
      await program.provider.connection.getAccountInfo(positionPubKey);
    ...
    const lbPair = await program.account.lbPair.fetch(position.lbPair());
    const binArrayKeys = position.getBinArrayKeysCoverage(program.programId);
    ...
    const mints = [
      lbPair.tokenXMint,
      lbPair.tokenYMint,
      ...lbPair.rewardInfos.map(({ mint }) => mint),
    ];
    const mintAccountInfos = await connection.getMultipleAccountsInfo(mints);
    const [tokenXMint, tokenYMint, reward0Mint, reward1Mint] =
      mintAccountInfos.map((accountInfo, index) => {
        const mint = mints[index];
        return unpackMint(mint, accountInfo, accountInfo.owner);
      });
    return {
      publicKey: positionPubKey,
      positionData: await DLMM.processPosition(
